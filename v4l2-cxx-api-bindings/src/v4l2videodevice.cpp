...
int V4L2VideoDevice::open(const std::string devicePath, const std::string subDevicePath)
{
...
int V4L2VideoDevice::setFormat(int pixelFormat)
{
        int ret = getFormat();
        if (ret == -1) {
                return -1;
...
int V4L2VideoDevice::printFormat()
{
        int ret = getFormat();
        if (ret == -1) {
                return -1;
...
int V4L2VideoDevice::setSelection(int left, int top, int width, int height)
{
        switch (m_format.type) {
        case V4L2_BUF_TYPE_VIDEO_CAPTURE:
...
int V4L2VideoDevice::streamOn(int bufferCount)
{
        int ret = getFormat();
        if (ret == -1) {
                return -1;
...
int V4L2VideoDevice::getNextImage(Image *&image, int timeout, bool lastImage)
{
        image = NULL;
...
int V4L2VideoDevice::releaseImage(Image *image)
{
        if (image != m_image) {
                return -1;
        }
...
int V4L2VideoDevice::getImage(Image *&image, int timeout, bool lastImage)
{
        int ret = streamOn(3);
        if (ret < 0) {
                return ret;
...
int V4L2VideoDevice::setExposure(int exposure)
{
        return setControl("Exposure", exposure);
}

int V4L2VideoDevice::setGain(int gain)
{
        return setControl("Gain", gain);
}

int V4L2VideoDevice::setBlackLevel(int blackLevel)
{
        return setControl("Black Level", blackLevel);
}

int V4L2VideoDevice::setBinning(int binning)
{
        return setControl("Binning", binning);
}

int V4L2VideoDevice::setTriggerMode(int triggerMode)
{
...
int V4L2VideoDevice::setControl(unsigned int id, int value)
{
...
int V4L2VideoDevice::setExtControl(unsigned int id, unsigned int type, int value)
{
...
int V4L2VideoDevice::setControl(std::string name, int value)
{
...
int V4L2VideoDevice::initBuffers(int bufferCount)
{
        clearBuffers();
...
void V4L2VideoDevice::clearBuffers()
{
        if (m_buffers == NULL) {
                return;
        }
...
void V4L2VideoDevice::handleErrorForOpen(const char *path, int err)
{
        char message[32];
...
void V4L2VideoDevice::handleErrorForClose(int fd, int err)
{
        char message[32];
...
void V4L2VideoDevice::handleErrorForSelect(int fd, int err)
{
        char message[32];
...
void V4L2VideoDevice::handleErrorForIoctl(unsigned long int request, int err)
{
        switch (request) {
